...
export default function AdminBlogsPage() {
  ...
  const stagedMedia = useStagedMedia({ multiple: false });
  ...
  const handleOpenEdit = (blog: BlogPost) => {
    setEditingBlog(blog);
    stagedMedia.setInitialMedia(blog.image);
    setFormData({
      title: blog.title,
      slug: blog.slug,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (stagedMedia.hasLocalFiles) {
        const { primaryUrl } = await stagedMedia.publishMedia();
        finalImageUrl = primaryUrl;
      } else if (stagedMedia.stagedItems.length > 0 && stagedMedia.stagedItems[0].source === "remote") {
        finalImageUrl = stagedMedia.stagedItems[0].url;
      }
      ...
      if (editingBlog) {
        ...
        if (json.success) {
          setIsEditModalOpen(false);
          await useBlogStore.getState().fetchBlogs(true);
          notifyContentUpdated("blogs");
          fetchBlogs();
          ...
          <div className="bg-white w-full max-w-4xl lg:max-w-5xl max-h-[88vh] sm:max-h-[85vh] rounded-sm border border-slate-200 shadow-2xl overflow-hidden flex flex-col my-auto shrink-0">
            ...
            <form onSubmit={handleSubmit} className="flex-1 flex flex-col min-h-0 overflow-hidden">
              <div className="flex-1 overflow-y-auto min-h-0 p-6 space-y-5 overscroll-contain" data-lenis-prevent>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1 leading-tight">Main Cover Image</label>
                  <MediaUploader
                    stagedMedia={stagedMedia}
                    label="Upload Main Cover Image"
                    helperText="Local preview displayed immediately. Image will be compressed and uploaded upon clicking Publish/Save."
                  />
                </div>
              ...
              {/* Footer with Left-Aligned Toggle Switch */}
              <div className="p-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between gap-4 shrink-0">
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-slate-900 ${
                      formData.published ? "bg-emerald-600" : "bg-slate-300"
                    }`}
                  >
                    <span
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-4 py-2 text-xs font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-sm cursor-pointer disabled:opacity-50 transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                  ...
                  >
                    {isSubmitting || stagedMedia.isUploading ? (
                      <>
                        ...
                        <span>{stagedMedia.isUploading ? "Uploading Media..." : "Saving..."}</span>
                      </>
                    ) : !formData.published ? (
                      "Save Draft"
                    ) : editingBlog ? (
